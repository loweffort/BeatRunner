...
public class GameManagerUpperBoundaryTesting : GameManager
{
    ...
    private GameManagerUpperBoundaryTesting() { }
    ...
    public static GameManager getInstance()
    {
        ...
        lock (padlock)
        {
            if (gameManagerInstance == null)
            {
                gameManagerInstance = new GameManagerUpperBoundaryTesting();
            }
            return gameManagerInstance;
        }
    }
    ...
    private StateUpperBoundaryTesting currentState;
    public void Request()
    {
        currentState.Handle(this);
    }
    public StateUpperBoundaryTesting State
    {
        get
        {
            return currentState;
    ...
    void Start()
    {
        ...
        currentState = new OpenUpperBoundaryTesting();
    ...
    private void OnGUI()
    {
        if (testSuccess)
        {
            GUI.Label(new Rect(80 - 12, 50, 100, 100), "Game Manager successfully managed max obstacles on hard!");
        }
        else
        {
            GUI.Label(new Rect(80 - 12, 50, 100, 100), "Game Manager could not manage max obstacles on hard!");
        }
    }
    ...
    void Update()
    {
        ...
        for (int i = 0; i < obstacleCount; i++)
        {
            obstacles[i].GetComponent<Rigidbody>().velocity = new Vector3(0, 0, -1 * 500);
...
public abstract class StateUpperBoundaryTesting
{
    public abstract void Handle(GameManagerUpperBoundaryTesting context);
    public abstract int getObstacleLayout();
}

//initial state class
public class OpenUpperBoundaryTesting : StateUpperBoundaryTesting
{
    public override void Handle(GameManagerUpperBoundaryTesting context)
    {
        //determine new obstacle orientation
        switch (Random.Range(0, 3))
        {
            case (0):
                context.State = new LeftUpperBoundaryTesting();
                break;
            case (1):
                context.State = new MiddleUpperBoundaryTesting();
                break;
            case (2):
                context.State = new RightUpperBoundaryTesting();
                break;
        }
    }
...
public class LeftUpperBoundaryTesting : StateUpperBoundaryTesting
{
    public override void Handle(GameManagerUpperBoundaryTesting context)
    {
        //determine new obstacle orientation
...
public class MiddleUpperBoundaryTesting : StateUpperBoundaryTesting
{
    public override void Handle(GameManagerUpperBoundaryTesting context)
    {
        //determine new obstacle orientation
...
public class RightUpperBoundaryTesting : StateUpperBoundaryTesting
{
    public override void Handle(GameManagerUpperBoundaryTesting context)
    {
        //determine new obstacle orientation
